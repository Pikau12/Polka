...
class SessionRepository @Inject constructor(
    ...
    fun getUserSessions(userId: Long): Flow<List<Session>> {
        return sessionDao.getUserSessions(userId).map{entities -> entities.map {entity -> entity.toModel()}}
    }

    fun getSessionsByGameId(gameId: Long): Flow<List<Session>> {
        return sessionDao.getSessionsByGameId(gameId).map{entities -> entities.map {entity -> entity.toModel()}}
    }
